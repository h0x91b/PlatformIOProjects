...
#define DEBUG 1          // отправка информации в порт, 1-вкл, 0-выкл
...
#define LED_PIN 7 // пин DIN ленты
...
void setup()
{
...
#endif
  if (startupTrigger == LOW)
  { // если при запуске кнопка НАЖАТА
    while (digitalRead(SW_ENC) == LOW)
    ...
    for (;;)
    {
      ...
      if (turn_flag)
      {
        ...
        Serial.println((int)norm_counter);
        turn_flag = false;
      }
      if (digitalRead(SW_ENC) == LOW)
        break;
    }
    NORM_STEP = 1;
  ...
  { // если кнопка НЕ НАЖАТА при старте - используем преобразованные картинки!
  ...
  if (maxLPS > 400)
    maxLPS = 400; // NeoPixel PWM rate is ~400 Hz
  if (DEBUG)
...
void loop()
{
  ...
  { // ждём нажатия кнопки !!!
    ...
    {
      if (SWstate)
      { // если НЕ НАЖАТА кнопка энкодера
  ...
  { // бесконечный цикл
    ...
    if (stopFlag)
      break; // выходим из цикла, если stopFlag

    if (++block >= nBlocks)
    { // показали последний блок
    ...
    uint8_t *brightness)
{
  ...
  if (brightness)
    ...
    return false; // MUST pass brightness for power est.
  ...
  { // Compression: must be 0 (none)
    ...
    { // Doing conversion?  Need outFile.
      ...
      ok = true; // outFile not needed; proceed
    ...
    { // Valid BMP and contig file (if needed) are ready
      ...
      if (bmpHeight < 0)
      {                         // If bmpHeight is negative,bmpHeight
      ...
      { // For each row in image...
        Serial.write('.');
        inFile.seekSet(
            bmpImageoffset + (bmpStartCol * 3) + (rowSize * (flip ? (bmpHeight - 1 - row) : // Image is stored top-to-bottom
                                                                 row)));                    // Image stored bottom-to-top
        if (!inFile.read(ledStartPtr, columns * 3))
        { // Load row
          if (DEBUG)
